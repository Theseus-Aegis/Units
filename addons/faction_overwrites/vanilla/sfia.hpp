// Units - SFIA
class I_SFIA_soldier_lxWS: I_soldier_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_7("20Rnd_762x51_slr_reload_tracer_green_lxWS"), mag_2("HandGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_7("20Rnd_762x51_slr_reload_tracer_green_lxWS"), mag_2("HandGrenade"), mag_2("SmokeShell")};
};
class I_SFIA_Soldier_TL_lxWS: I_Soldier_TL_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("20Rnd_762x51_slr_reload_tracer_green_lxWS"), mag_3("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("20Rnd_762x51_slr_reload_tracer_green_lxWS"), mag_3("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
};
class I_SFIA_Soldier_GL_lxWS: I_Soldier_GL_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("20Rnd_762x51_slr_reload_tracer_green_lxWS"), mag_3("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell"), mag_3("1Rnd_40mm_HE_lxWS"), mag_2("1Rnd_58mm_AT_lxWS"), "1Rnd_50mm_Smoke_lxWS"};
    //respawnMagazines[] = {mag_6("20Rnd_762x51_slr_reload_tracer_green_lxWS"), mag_3("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell"), mag_3("1Rnd_40mm_HE_lxWS"), mag_2("1Rnd_58mm_AT_lxWS"), "1Rnd_50mm_Smoke_lxWS"};
};
class I_SFIA_Soldier_AR_lxWS: I_Soldier_AR_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_5("tacgt_AI_30Rnd_762x39_AK"), mag_2("HandGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_5("tacgt_AI_30Rnd_762x39_AK"), mag_2("HandGrenade"), mag_2("SmokeShell")};
};
class I_SFIA_officer_lxWS: I_officer_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("20Rnd_762x51_slr_reload_tracer_green_lxWS"), mag_3("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("20Rnd_762x51_slr_reload_tracer_green_lxWS"), mag_3("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
};
class I_SFIA_sharpshooter_lxWS: I_G_Sharpshooter_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("20Rnd_762x51_slr_reload_tracer_green_lxWS"), mag_3("tacgt_AI_17Rnd_9x19_Walther"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("20Rnd_762x51_slr_reload_tracer_green_lxWS"), mag_3("tacgt_AI_17Rnd_9x19_Walther"), mag_2("SmokeShell")};
};
class I_SFIA_crew_lxWS: I_crew_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("tacgt_AI_35Rnd_556x45_R4"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("tacgt_AI_35Rnd_556x45_R4"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
};
class I_SFIA_pilot_lxWS: I_pilot_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_2("tacgt_AI_30Rnd_9x19")};
    //respawnMagazines[] = {mag_2("tacgt_AI_30Rnd_9x19")};
};
class I_SFIA_soldier_at_lxWS: I_Soldier_AT_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_5("tacgt_AI_30Rnd_762x39_AK"), "RPG32_F", mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_5("tacgt_AI_30Rnd_762x39_AK"), "RPG32_F", mag_2("SmokeShell")};
};
class I_SFIA_soldier_aa_lxWS: I_Soldier_AA_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_5("tacgt_AI_30Rnd_762x39_AK"), "Titan_AA", mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_5("tacgt_AI_30Rnd_762x39_AK"), "Titan_AA", mag_2("SmokeShell")};
};
class I_SFIA_medic_lxWS: I_G_medic_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_8Rnd_45ACP_1911"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_8Rnd_45ACP_1911"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
};
class I_SFIA_exp_lxWS: I_G_Soldier_exp_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_3("APERSMine_Range_Mag"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_3("APERSMine_Range_Mag"), mag_2("SmokeShell")};
};
class I_SFIA_repair_lxWS: I_Soldier_repair_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
};
class I_SFIA_Soldier_AAT_lxWS: I_Soldier_AAT_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
};
class I_SFIA_Soldier_AAA_lxWS: I_Soldier_AAA_F {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
};
class I_SFIA_soldier_lite_lxWS: I_SFIA_soldier_lxWS {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
};
class O_SFIA_HeavyGunner_lxWS: O_SFIA_Soldier_AR_lxWS {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_5("100Rnd_762x51_S77_Yellow_lxWS"), mag_2("tacgt_AI_8Rnd_45ACP_1911"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_5("100Rnd_762x51_S77_Yellow_lxWS"), mag_2("tacgt_AI_8Rnd_45ACP_1911"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
};
class O_SFIA_soldier_lite_lxWS: O_SFIA_soldier_lxWS {
    scope = 1;
    scopeCurator = 1;
    //magazines[]mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
    //respawnMagazines[] = {mag_6("tacgt_AI_30Rnd_762x39_AK"), mag_2("tacgt_AI_17Rnd_9x19_Walther"), mag_2("MiniGrenade"), mag_2("SmokeShell")};
};

// Opfor
class O_SFIA_Soldier_AAA_lxWS: I_SFIA_Soldier_AAA_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Soldier_AAT_lxWS: I_SFIA_Soldier_AAT_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Soldier_AR_lxWS: I_SFIA_Soldier_AR_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_medic_lxWS: I_SFIA_medic_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_crew_lxWS: I_SFIA_crew_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_exp_lxWS: I_SFIA_exp_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Soldier_GL_lxWS: I_SFIA_Soldier_GL_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_HeavyGunner_lxWS: O_SFIA_Soldier_AR_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_soldier_aa_lxWS: I_SFIA_soldier_aa_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_soldier_at_lxWS: I_SFIA_soldier_at_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_officer_lxWS: I_SFIA_officer_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_pilot_lxWS: I_SFIA_pilot_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_repair_lxWS: I_SFIA_repair_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_soldier_lxWS: I_SFIA_soldier_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_soldier_lite_lxWS: O_SFIA_soldier_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_soldier_unarmed_lxWS: I_SFIA_soldier_unarmed_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_sharpshooter_lxWS: I_SFIA_sharpshooter_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_survivor_lxWS: I_SFIA_survivor_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Soldier_TL_lxWS: I_SFIA_Soldier_TL_lxWS {
    scope = 1;
    scopeCurator = 1;
};

// Vehicles
class O_SFIA_Truck_02_aa_lxWS: I_SFIA_Truck_02_aa_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_APC_Tracked_02_AA_lxWS: I_SFIA_APC_Tracked_02_AA_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_APC_Tracked_02_cannon_lxWS: I_SFIA_APC_Tracked_02_cannon_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_APC_Tracked_02_30mm_lxWS: I_SFIA_APC_Tracked_02_30mm_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_APC_Wheeled_02_hmg_lxWS: I_SFIA_APC_Wheeled_02_hmg_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_APC_Wheeled_02_unarmed_lxWS: I_SFIA_APC_Wheeled_02_unarmed_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Truck_02_MRL_lxWS: I_SFIA_Truck_02_MRL_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Truck_02_fuel_lxWS: I_SFIA_Truck_02_fuel_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Truck_02_transport_lxWS: I_SFIA_Truck_02_transport_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Truck_02_covered_lxWS: I_SFIA_Truck_02_covered_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Offroad_lxWS: I_SFIA_Offroad_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Offroad_AT_lxWS: I_SFIA_Offroad_AT_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Offroad_armed_lxWS: I_SFIA_Offroad_armed_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Truck_02_Ammo_lxWS: I_SFIA_Truck_02_Ammo_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Truck_02_cargo_lxWS: I_SFIA_Truck_02_cargo_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Truck_02_flatbed_lxWS: I_SFIA_Truck_02_flatbed_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Truck_02_box_lxWS: I_SFIA_Truck_02_box_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Heli_Attack_02_dynamicLoadout_lxWS: I_SFIA_Heli_Attack_02_dynamicLoadout_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_MBT_02_cannon_lxWS: I_SFIA_MBT_02_cannon_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_HMG_02_lxWS: I_SFIA_HMG_02_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_HMG_02_high_lxWS: I_SFIA_HMG_02_high_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_Mortar_lxWS: I_SFIA_Mortar_lxWS {
    scope = 1;
    scopeCurator = 1;
};
class O_SFIA_ZU23_lxWS: I_SFIA_ZU23_lxWS {
    scope = 1;
    scopeCurator = 1;
};
